(function () {
    cc.PhoenixBetView = cc.Class({
            ...
            switch (info.Phrase) {
                ...
                case cc.PhoenixState.BETTING:
                    if (this.isBeted && !this.isBetLater) {
                        ...
                        // PHANH SO DU: truoc day auto-bet cu ban lenh cuoc MOI PHA BETTING ma khong
                        // kiem tien. Het tien thi moi van server tra ve loi -> bang chu do dinh man hinh,
                        // van sau lai ban tiep, nguoi choi phai tu tay tat auto moi thoat. Sunwin co phanh nay.
                        if (!this.hasEnoughBalance(this.betValue)) {
        ...
        // Dung chung API so du cua repo (giong chooseBetValue ben duoi), KHONG dung so du chup luc login.
        hasEnoughBalance: function (amount) {
            var balance = cc.BalanceController.getInstance().getBalance();
            return parseInt(amount || 0) <= balance;
        },
        ...
        sendRequestBet: function (betValue) {
            ...
            if (this.controller.playPhx) this.controller.playPhx(cc.AudioTypes.PHX_SFX_BET);
        ...
        resetBetValue: function () {
            if (this.phrase == cc.PhoenixState.FLYING && !this.isCashout && this.isBeted) {
                return;
            }
        ...
        updateWinPreview: function (multiplier) {
            if (this.phrase !== cc.PhoenixState.FLYING) return;
        ...
        setCashout: function (isCashout) {
            ...
            if (!isCashout || this.phrase !== cc.PhoenixState.FLYING) return;
