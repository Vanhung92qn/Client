...
(function () {
    cc.PhoenixView = cc.Class({
        ...
        onDestroy: function () {
            ...
            if (cc.sys.isNative) {
                cc.assetManager.releaseAsset('phoenix');
            }
            cc.PopupController.getInstance().hideBusy();
        },
        ...
        sendRequestOnHub: function (method, data) {
            switch (method) {
                ...
                case cc.MethodHubName.SEND_MESSAGE:
                    ...
                    break;
                case cc.MethodHubName.GET_USER_IN_ROOM:
                    this.AviatorHub.getUserInRoom();
                    break;
            }
        },
                ...
                res.map(m => {
                    switch (m.M) {
                        ...
                        case cc.MethodHubOnName.JOIN_GAME:
                            ...
                            if (myBet && !data.IsCrashed && this.controller.getCurrentState() === cc.PhoenixState.FLYING && !myBet.IsWon && (myBet.Award | 0) <= 0) {
                                this.controller.setBeted(true);
                                this.controller.updateBetView({ Phrase: cc.PhoenixState.FLYING });
                            }
                        ...
                        case cc.MethodHubOnName.CASH_OUT_SUCCESS:
                            if (m.A[0] > 0) {   //A[0]=tiền thắng (guard), A[1]=SỐ DƯ thật sau khi cộng
                                ...
                                this.lbBalance.tweenValueto(m.A[1]);
                                cc.BalanceController.getInstance().updateRealBalance(m.A[1]);
                                // Tieng RUT THANG: cam o day (nhanh cua CHINH MINH), KHONG cam vao
                                // PLAYER_CASH_OUT vi kenh do la ca phong -> se keu moi lan bot rut.
                                if (this.controller.playPhx) this.controller.playPhx(cc.AudioTypes.PHX_SFX_TAKE_PROFIT);
                            }
                            break;
                        //Thong bao
                        ...
                        case cc.MethodHubOnName.JACKPOT_WINNER:
                            ...
                            break;
                        case cc.MethodHubOnName.FEATHER_DROP:
                            this.controller.updateFeather(m.A[0]);   // HOA PHUNG: long vu roi (FeatherCount)
                            break;
                        case cc.MethodHubOnName.FLYING_EVENT:
                            this.controller.onFlyingEvent(m.A[0], m.A[1], m.A[2], m.A[3], m.A[4]);   // HOA PHUNG: bien dao (cL,aL,bId,aId,fc)
                            break;

                    }
                });

            } else if (response.R && response.R.Users !== undefined) {
                // Ket qua GetUserInRoom (phan biet theo HINH DANG cua R - dung loi co san cua file nay).
                this.controller.updateUserInRoom(response.R.Users, response.R.Total);
        ...
        showJackpotBroadcast: function (amount, winnersJson) {
            // HIEU UNG NO HU chay cho MOI nguoi (dat TRUOC cac nhanh return ben duoi):
            // hu jackpot_sunphung `in`->`jackpot`, 5 khe long vu chop `jackpot`, dan chim `idle_to_win_birds`.
            this.controller.playJackpotAnim();
            var winners = [];
