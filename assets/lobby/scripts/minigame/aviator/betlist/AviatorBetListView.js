    ...
    cc.AviatorBetListView = cc.Class({
        ...
        initialize: function (messages, iscrashed) {
            ...
            for (var i = 0; i < spawnCountReal; ++i) {
                ...
                item.getComponent(cc.AviatorBetItem).updateItem(this.messages[i], i, iscrashed);
            ...
            for (var i = 0; i < items.length; ++i) {
                ...
                if (isDown) {
                    if (viewPos.y < -buffer && items[i].y + offset < 0) {
                        ...
                        var item = items[i].getComponent(cc.AviatorBetItem);
                        var itemId = item.itemID - items.length;
                        item.updateItem(this.messages[itemId], itemId, iscrashed);
                    }
                } else {
                    if (viewPos.y > buffer && items[i].y - offset > -this.content.height) {
                        items[i].y = items[i].y - offset;
                        item = items[i].getComponent(cc.AviatorBetItem);
