...
function baoDamMusicPlayer() {
    ...
    var node = new cc.Node('CardMusicPlayer');
...
function caiChuyenHuongCanh() {
    if (cc.director.__caRuaDaVaChuyenCanh) return;
    cc.director.__caRuaDaVaChuyenCanh = true;
    ...
    cc.director.loadScene = function (tenCanh, onLaunched, onUnloaded) {
        if (!laCanhGoc(tenCanh)) return loadGoc(tenCanh, onLaunched, onUnloaded);
    ...
    cc.director.preloadScene = function (tenCanh, onProgress, onLoaded) {
        if (!laCanhGoc(tenCanh)) return preloadGoc(tenCanh, onProgress, onLoaded);
